...
// Get rid of that cruddy frontend WP bar, cause fuck it.
...
/**
 * Pull User Data & Game Stats
 *
 * 
 */
 add_filter('manage_users_columns', 'user_id_column');
function user_id_column($columns) {
    $columns['user_id'] = 'User ID';
    return $columns;
}
 add_filter('manage_users_columns', 'user_codex_column');
function user_codex_column($columns) {
    $columns['user_codex'] = 'Codex Score';
    return $columns;
}
...
{
	$api_key = '7616E3E6A79141CD';
	$base = get_option('json_api_base', 'api');
	$this_url = "http://$_SERVER";
	
	if ( ( isset($_REQUEST['json'] ) || strstr($this_url , $base.'/') ) && ( !isset($_REQUEST['apikey']) || $_REQUEST['apikey'] != '7616E3E6A79141CD' ) )
	{
...
echo '<li class="right nav_identity cf" id="top-notification"> <div class="nav-bucket">';
_e( '<i class="fa fa-bell" style="text-align:center; font-size:30px; float:left; padding-top: 15px;"></i>', 'buddypress' );

if ( $notifications = bp_core_get_notifications_for_user( $bp->loggedin_user->id ) ) { ?>
    <span style="color: #6699ff; font-size:16px; z-index:2; position:absolute; margin-right: 10px; margin-top: 5px; padding-right: 10px; padding-top: 5px;"><div class="circle"><?php echo count( $notifications ) ?></div></span>
<?php
}
...
<?php
function my_bp_adminbar_messages_menu() {
global $bp;
...
echo '<li class="right nav_identity cf" id="top-notification"> <div class="nav-bucket">';
_e( '<i class="fa fa-envelope" style="text-align:center; font-size:30px; float:left; padding-top: 15px;"></i>', 'buddypress' );

if ( $messages = bp_get_total_unread_messages_count( $bp->loggedin_user->id ) ) { ?>
    <span style="color: #6699ff; font-size:16px; z-index:2; position:absolute; margin-right: 10px; margin-top: 5px; padding-right: 10px; padding-top: 5px;"><div class="circle"><?php echo count( $messages ) ?></div></span>
<?php
}
...
<?php
function my_bp_adminbar_friends_menu() {
global $bp;
...
echo '<li class="right nav_identity cf" id="top-notification"> <div class="nav-bucket">';
_e( '<i class="fa fa-users" style="text-align:center; font-size:30px; float:left; padding-top: 15px;"></i>', 'buddypress' );

if ( $friends = bp_friend_get_total_requests_count( $bp->loggedin_user->id ) ) { ?>
<span style="color: #6699ff; font-size:16px; z-index:2; position:absolute; margin-right: 10px; margin-top: 5px; padding-right: 10px; padding-top: 5px;"><div class="circle"><?php echo count( $friends ) ?></div></span>
<?php
}
